...
export async function POST(request: Request) {
  try {
    ...
    // Restrict access strictly to 'advanced' only
    if (client.translationType !== 'advanced') {
      return NextResponse.json({ error: 'Client does not have access to advanced translation' }, { status: 403 });
    }
...
- Do not include Markdown, code blocks, or language annotations (e.g., \`\`\`html).
